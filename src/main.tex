...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If $g^{-1}(x) = \{a\}$ and $g^{-1}(x') = \empty$, then we have picked $y_x$ such that
      $f(a) < y_x$ so $g'(x) = (f(a),x) < (y_x, x) = g'(x')$.
    \item If $g^{-1}(x) = \empty$ and $g^{-1}(x') = \{a\}$, then we have picked $y_x$ such that
      $y_x < f(a)$ so $g'(x) = (y_x, x) < (f(a),x) = g'(x')$.
    \item If $g^{-1}(x) = g^{-1}(x') = \empty$ then notice that $y_x \leq y_{x'}$ since
